...
function formatMoney(entries: Array<{ currency: string; amount: number }>) {
  ...
  return entries.map((e) => `${e.currency === 'INR' ? '৳' : e.currency} ${Math.round(e.amount).toLocaleString('bn-BD')}`).join(' + ')
}
      ...
      {loading ? (
        ...
        >
          ...
          <View style={styles.grid}>
            <StatTile label="আজকের অর্ডার" value={String(data?.data.commerce.ordersToday ?? 0)} hint={`${data?.data.commerce.paidOrdersToday ?? 0} পরিশোধিত`} />
            <StatTile label="আজকের আয়" value={formatMoney(data?.data.commerce.revenueToday ?? [])} />
            <StatTile label="অপেক্ষমাণ পেমেন্ট" value={String(data?.data.commerce.ordersAwaitingPayment ?? 0)} tone="warning" />
          ...
          <View style={styles.grid}>
            <StatTile label="মডারেশন সারি" value={String(data?.data.editorial.pendingModeration ?? 0)} tone={data && data.data.editorial.pendingModeration > 0 ? 'warning' : 'default'} />
            <StatTile label="স্টল আবেদন" value={String(data?.data.editorial.pendingApplications ?? 0)} tone={data && data.data.editorial.pendingApplications > 0 ? 'warning' : 'default'} />
            <StatTile label="খোলা রিপোর্ট" value={String(data?.data.editorial.openReports ?? 0)} tone={data && data.data.editorial.openReports > 0 ? 'error' : 'default'} />
          </View>

          <Text style={styles.sectionTitle}>প্ল্যাটফর্ম</Text>
          <View style={styles.grid}>
            <StatTile label="মোট ব্যবহারকারী" value={String(data?.data.people.total ?? 0)} hint={`+${data?.data.people.newToday ?? 0} আজ`} />
            <StatTile label="স্টল" value={String(data?.data.marketplace.organizations ?? 0)} />
